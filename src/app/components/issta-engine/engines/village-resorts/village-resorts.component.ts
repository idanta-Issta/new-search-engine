...
export class VillageResortsComponent extends BaseEngineComponent {
    ...
    switch (type) {
      ...
      case ESharedInputType.VILLAGE_RESORTS_DATES:
        if (value?.start && value?.end) {
          this.inputsRow?.openInputDelayed(ESharedInputType.PASSANGERS_OPTIONS);
        }
        break;
    }
  }
  ...
  buildUrl(): string {
    ...
    if (this.selectedDestination?.CityCode) {
      params.push(`dcode=${this.selectedDestination.CityCode}`);
    }
